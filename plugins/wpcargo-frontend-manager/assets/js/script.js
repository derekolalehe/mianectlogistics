jQuery(document).ready(function($){
    ...
    */
    ...
    //TW Custom
    $('select[data-filter="receiverid"]').select2({});
    //
    ...
    var donwloadWaybill   = '<button class="donwloadWaybill btn btn-success btn-sm><i class="fa fa-file-pdf text-white"></i> '+downloadLabel+'</button>';
    ...
    $('#shipments-table-list').on('click', '.donwloadWaybill', function(e){
        e.preventDefault();
    ...
    $('.wpcargo-dashboard').on('click', '#donwloadWaybillSingle', function(e){
        e.preventDefault();
        var selectedShipment = [];
        selectedShipment.push( $(this).data('id') );
        $.ajax({
            type:"POST",
            data:{
                action  : 'wpcfe_downloadwaybill',    
    ...
    $('#shipmentBulkUpdateModal').on('submit', '#shipmentBulkUpdate-form', function( e ){
        ...
        var employee  = $('#shipmentBulkUpdateModal #registered_employee').val();
        var client    = $('#shipmentBulkUpdateModal #registered_client').val();
        var agent     = $('#shipmentBulkUpdateModal #registered_agent').val();
