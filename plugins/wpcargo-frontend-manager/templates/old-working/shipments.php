					...
					<table id="shipment-list" class="table table-hover table-sm">
						...
						<tbody>
							...
							while ( $wpc_shipments->have_posts() ) {
								...
								$status 		= $wpdb->get_row("SELECT meta_value FROM " . $wpdb->prefix . "postmeta WHERE meta_key = 'wpcargo_status' AND post_id = " . get_the_ID());//get_post_meta( get_the_ID(), 'wpcargo_status', true );
								...
								?>
								<tr id="shipment-<?php echo get_the_ID(); ?>" class="shipment-row">
									<td class="form-check">
									...
									<td class="shipment-status"><?php echo $status->meta_value;?></td>
